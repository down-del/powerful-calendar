...
class EventStorage {
  Future<List<Map<String, dynamic>>> loadEvents() async {
    ...
    decodedList.sort((a, b) {
      bool aValue = a['completed'] as bool;
      bool bValue = b['completed'] as bool;

      return (aValue ? 1 : 0).compareTo(bValue ? 1 : 0); // 升序排列
    });
                ...
                onTap: () {
                  setState(() {
                    if (_isSelectionMode) {
                      ...
                      // 重新排序事件列表，已完成的事件放到列表的後面(但會亂掉)
                      eventsList.sort((a, b) {
                        bool aValue = a['completed'] as bool;
                        bool bValue = b['completed'] as bool;

                        return (aValue ? 1 : 0)
                            .compareTo(bValue ? 1 : 0); // 升序排列
                      });
