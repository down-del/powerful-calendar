...
class _CalendarWidgetState extends State<CalendarWidget> {
  ...
  int _firstDayOffset() {
    DateTime firstDay =
        DateTime(_currentMonth.year, _currentMonth.month, 1); // 获取当前月的第一天
  ...
  void _previousMonth() {
    setState(() {
      _currentMonth = DateTime(
          _currentMonth.year, _currentMonth.month - 1); // 当前月份减去1，变成上个月
    });
  }
  ...
  void _nextMonth() {
    setState(() {
      _currentMonth =
          DateTime(_currentMonth.year, _currentMonth.month + 1); // 当前月份加1，变成下个月
    });
  }
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                          ...
                          decoration: isCompleted
                              ...
                              : TextDecoration.none, // 如果事件已完成，加上删除线
                        ),
                      ),
                    );
                  }).toList(),
          ...
          Row(
            ...
            children: [
              ...
              Padding(
                ...
                child: Text(
                  DateFormat.yMMM()
                      .format(_currentMonth), // 显示当前月份和年份，格式为 "yyyy MMM"
            ...
            child: FutureBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                return CustomScrollView(
                  // 使用 CustomScrollView 让页面内容能够垂直滚动
                  slivers: [
                    // SliverGrid 是一个可以让内容网格化的滚动组件
                    SliverGrid(
                      // gridDelegate 决定了每一行的布局和格子的大小
                      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                        crossAxisCount: 7, // 每行显示 7 个格子，表示一周的 7 天
                        childAspectRatio: 0.62, // 设置每个格子的宽高比，0.62 让格子更为接近正方形
                        mainAxisSpacing: 8, // 每个格子之间的垂直间距
                        crossAxisSpacing: 8, // 每个格子之间的水平间距
                      ),
                      // SliverChildBuilderDelegate 是一个生成网格子项的代理
                      delegate: SliverChildBuilderDelegate(
                        (BuildContext context, int index) {
                          // 计算当前格子的日期
                          int day = index - _firstDayOffset() + 1;

                          // 如果 day <= 0 或 day > 当前月份的天数，说明该格子是空的
                          if (day <= 0 || day > _daysInMonth(_currentMonth)) {
                            return SizedBox.shrink(); // 返回一个空的空间占位符
                          }
                          ...
                          return GestureDetector(
                            onTap: () => _showEventDetails(
                                context, day, eventsForDay), // 点击格子时显示该日期的事件详情
                            child: Container(
                              margin: EdgeInsets.all(1), // 设置每个格子的外边距
                              decoration: BoxDecoration(
                                color: isToday
                                    ? Colors.blue
                                    : Colors.transparent, // 如果是今天，背景设置为蓝色
                                borderRadius:
                                    BorderRadius.circular(8), // 设置圆角边框
                                // border: Border.all(color: const Color.fromARGB(255, 255, 255, 255)), // 设置格子的边框颜色
                              ),
                              child: Column(
                                mainAxisAlignment:
                                    MainAxisAlignment.start, // 让内容垂直居中
                                children: [
                                  // 显示日期
                                  Text(
                                    '$day', // 显示当前日期
                                    style: TextStyle(
                                      fontWeight: FontWeight.bold, // 日期加粗
                                      fontSize: 16, // 日期的字体大小
                                      color: isToday
                                          ? Colors.white
                                          : Colors.black, // 如果是今天，文字颜色为白色，否则为黑色
                                    ),
                                  ),
                                  // 如果当天有事件，显示事件图标
                        ...
                        // childCount 是网格中项目的总数量，这里是当前月份的天数加上第一天的偏移量
                        childCount:
                            _daysInMonth(_currentMonth) + _firstDayOffset(),
                      ),
                    ),
                  ],
                );
